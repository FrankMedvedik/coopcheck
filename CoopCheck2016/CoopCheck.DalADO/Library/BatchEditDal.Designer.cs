...
namespace CoopCheck.DalADO.Library
{
    ...
    public partial class BatchEditDal : IBatchEditDal
    {
        ...
        public BatchEditDto Fetch(int batch_num)
        {
            ...
            {
                using (var cmd = new SqlCommand("dbo.GetBatchEdit", ctx.Connection))
                {
        ...
        public BatchEditDto Insert(BatchEditDto batchEdit)
        {
            using (var ctx = ConnectionManager<SqlConnection>.GetManager("MyDatabase"))
            {
                using (var cmd = new SqlCommand("dbo.AddBatchEdit", ctx.Connection))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
        ...
        public BatchEditDto Update(BatchEditDto batchEdit)
        {
            using (var ctx = ConnectionManager<SqlConnection>.GetManager("MyDatabase"))
            {
                using (var cmd = new SqlCommand("dbo.UpdateBatchEdit", ctx.Connection))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
        ...
        public void Delete(int batch_num)
        {
            using (var ctx = ConnectionManager<SqlConnection>.GetManager("MyDatabase"))
            {
                using (var cmd = new SqlCommand("dbo.DeleteBatchEdit", ctx.Connection))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.AddWithValue("@batch_num", batch_num).DbType = DbType.Int32;
                    var rowsAffected = cmd.ExecuteNonQuery();
